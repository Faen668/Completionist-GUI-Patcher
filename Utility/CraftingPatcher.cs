...
{
    internal class CraftingPatcher
    {
        public static void Patch(Patcher instance, string? filePath)
        {
            ...
            string actionScriptFile = Path.Combine(extractionDir, "scripts", "__Packages", "CraftingListEntry.as");
            string scriptName = @"\__Packages\CraftingListEntry";
            ...
            // Step 2: Read ActionScript file (CraftingListEntry.as)
            try
            {
            ...
            // Step 3: Modify ActionScript file (CraftingListEntry.as)
            try
            {
                if (string.IsNullOrEmpty(scriptContent))
                {

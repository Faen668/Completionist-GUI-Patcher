...
{
    public static class LootMenuPatcher
    {
        public static void Patch(Patcher instance, string? filePath)
        {
            ...
            string actionScriptFile = Path.Combine(extractionDir, "scripts", "__Packages", "LootMenu", "ListItemRenderer.as");
            string scriptName = @"\__Packages\LootMenu\ListItemRenderer";
            ...
            // Step 2: Read ActionScript file (ListItemRenderer.as)
            try
            {
            ...
            // Step 3: Modify ActionScript file (ListItemRenderer.as)
            try
            {
                if (string.IsNullOrEmpty(scriptContent))
                {
                ...
                // Define the regex pattern to find the line with 'var _locX_ = this.data.displayName == null ? "" : this.data.displayName;'
                string pattern = @"var\s+([_a-zA-Z0-9]+)\s*=\s*this\.data\.displayName\s*==\s*null\s*\?\s*""""\s*:\s*this\.data\.displayName;";
      ...
      if(_comp_[1].length > 0)
      {
         this.textField.textColor = parseInt(_comp_[1]);
                        ...
                        string targetLine = "_ = this.data.displayName == null ? \"\" : this.data.displayName;";
                        ...
                        string colorInsertString = "this.textField.textColor = ";
